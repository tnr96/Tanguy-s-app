...
moose_dir = '/Users/ag2556/Documents/CornellPostdoc/Cornell_Projects/MicroDescriptors/revised_simulations/app-results' 
simulation_dir = '/Users/ag2556/Documents/CornellPostdoc/Cornell_Projects/MicroDescriptors/revised_simulations/app-results'  # Where the different simulations are stored
#simulation_dir = '/Users/ag2556/Documents/CornellPostdoc/Cornell_Projects/MicroDescriptors/App_output/Simulations' 
...
cutoff_input = 200 # changed from 160
...
#correlation_descriptors = ['Two-point autocorrelation function', 'Two-point cluster function','Lineal-path function','Chord-length probability density function']


samples_names = ['REAL','SYNO']
perturbations_names = ['ANI-20','ISO-20','DEL-20']
...
def compute_error_correlation_fullname(t1, t2, dir1, dir2, m, n) :
    ...
    emd[0] = wasserstein_distance_nd(auto_correlation1[:,:], auto_correlation2[:,:])
...
def plot_errorpropag_fullname(fen, name, dir_to_save, groundtruth=None) :
    ...
    for c in coefficients :
        ...
        descriptors.remove('compared_tensors.txt')
...
def rank_descriptors_file_errorpropag(name, coeff, strings_input=[]) :
    ...
    label_spearman = ["".join(s.split('Spearman')[0].split(':')[1:3])  for i, s in enumerate(reversed(string_list_spearman))]
    
    label_spearman = convert_moment(label_spearman)
    new_label_spearman = label_spearman
    for i in range(len(label_spearman)):
        if label_spearman[i] == ' S2 ':
            new_label_spearman[i] = 'Two-point autocorrelation function'
        elif label_spearman[i] == ' Lineal Path ':
            new_label_spearman[i] = 'Lineal-path function'
        elif label_spearman[i] == ' Cluster ':
            new_label_spearman[i] = 'Two-point cluster function'
        elif label_spearman[i] == ' Angular Chord Length Distribution ':
            new_label_spearman[i] = 'Chord-length probability density function'
        elif label_spearman[i] == ' Size Mean ':
            new_label_spearman[i] = 'Mean of size'
        elif label_spearman[i] == ' Size Variance ':
            new_label_spearman[i] = 'Variance of size'
        elif label_spearman[i] == ' Size Skewness ':
            new_label_spearman[i] = 'Skewness of size'
        elif label_spearman[i] == ' Size ':
            new_label_spearman[i] = 'Probability density of size'
        elif label_spearman[i] == ' Rectangularity Mean ':
            new_label_spearman[i] = 'Mean of rectangularity'
        elif label_spearman[i] == ' Rectangularity Variance ':
            new_label_spearman[i] = 'Variance of rectangularity'
        elif label_spearman[i] == ' Rectangularity Skewness ':  
            new_label_spearman[i] = 'Skewness of rectangularity'
        elif label_spearman[i] == ' Rectangularity ':
            new_label_spearman[i] = 'Probability density of rectangularity'
        elif label_spearman[i] == ' Orientation Mean ':
            new_label_spearman[i] = 'Mean of orientation'
        elif label_spearman[i] == ' Orientation Variance ':
            new_label_spearman[i] = 'Variance of orientation'
        elif label_spearman[i] == ' Orientation Skewness ':
            new_label_spearman[i] = 'Skewness of orientation'
        elif label_spearman[i] == ' Orientation ':
            new_label_spearman[i] = 'Probability density of orientation'
        elif label_spearman[i] == ' Aspect ratio Mean ':
            new_label_spearman[i] = 'Mean of aspect ratio'
        elif label_spearman[i] == ' Aspect ratio Variance ':
            new_label_spearman[i] = 'Variance of aspect ratio'
        elif label_spearman[i] == ' Aspect ratio Skewness ':
            new_label_spearman[i] = 'Skewness of aspect ratio'
        elif label_spearman[i] == ' Aspect ratio ':
            new_label_spearman[i] = 'Probability density of aspect ratio'
        elif label_spearman[i] == ' Solidity Mean ':
            new_label_spearman[i] = 'Mean of solidity'
        elif label_spearman[i] == ' Solidity Variance ':
            new_label_spearman[i] = 'Variance of solidity'
        elif label_spearman[i] == ' Solidity Skewness ':
            new_label_spearman[i] = 'Skewness of solidity'
        elif label_spearman[i] == ' Solidity ':
            new_label_spearman[i] = 'Probability density of solidity'
        elif label_spearman[i] == ' Volume fraction ':
            new_label_spearman[i] = 'Volume fraction'
    
    # Here I want to change and plot absolute values of correlation coefficients
    corr_list_spearman = [abs(ele) for ele in corr_list_spearman]
    #bars = plt.barh(label_spearman[::-1], corr_list_spearman, label=label_spearman, color='skyblue')
    bars = plt.barh(new_label_spearman[::-1], corr_list_spearman, label=new_label_spearman, color='skyblue')
    ...
    for bar in bars:
        width = bar.get_width()
        label_x_pos = width
        plt.text(label_x_pos, bar.get_y() + bar.get_height()/2, f'{width:.2f}', va='center', fontsize = 10)
    
    
    plt.xticks(np.arange(0, 1.2, 0.2), labels=['0', '0.2', '0.4', '0.6', '0.8','1'])
...
def rank_descriptors_file(name, color, coeff, marker, label, strings_input=[]) :
    ...
    for path in consistent_tensors_full :    
        ...
        print('cluster filename being read ', os.path.join(img_char_path, tensor_name + '_cluster.npy'))
        print('shape of cluster is', cluster.shape)
    ...
    x = np.arange(1, len(corr_list_spearman)+1, 1)
    old_legend = convert_moment([' ' +  "".join(s.split('Spearman')[0].split(':')[1:3])  for i, s in enumerate(reversed(string_list_spearman))])
    new_legend = old_legend
    #print('legend to change:', old_legend)
    for i in range(len(old_legend)):
        if old_legend[i] == ' Volume fraction ':
            new_legend[i] = str(i+1) + ' Volume fraction'
        elif old_legend[i] == ' Cluster ':
            new_legend[i] = str(i+1) + ' Frobenius norm of two-point cluster function'
        elif old_legend[i] == ' Lineal Path ':
            new_legend[i] = str(i+1) + ' Frobenius norm of lineal-path function'
        elif old_legend[i] == ' S2 ':
            new_legend[i] = str(i+1) + ' Frobenius norm of two-point autocorrelation function'
        elif old_legend[i] == ' CLDa ':
            new_legend[i] = str(i+1) + ' Frobenius norm of chord-length probability density \n function'
        elif old_legend[i] == ' Size Mean ':
            new_legend[i] = str(i+1) + ' Mean of size'
        elif old_legend[i] == ' Size Variance ':
            new_legend[i] = str(i+1) + ' Variance of size'
        elif old_legend[i] == ' Size Skewness ':
            new_legend[i] = str(i+1) + ' Skewness of size'
        elif old_legend[i] == ' Orientation Mean ':
            new_legend[i] = str(i+1) + ' Mean of orientation'
        elif old_legend[i] == ' Orientation Variance ':
            new_legend[i] = str(i+1) + ' Variance of orientation'
        elif old_legend[i] == ' Orientation Skewness ':
            new_legend[i] = str(i+1) + ' Skewness of orientation'
        elif old_legend[i] == ' Aspect ratio Mean ':
            new_legend[i] = str(i+1) + ' Mean of aspect ratio'
        elif old_legend[i] == ' Aspect ratio Variance ':
            new_legend[i] = str(i+1) + ' Variance of aspect ratio'
        elif old_legend[i] == ' Aspect ratio Skewness ':
            new_legend[i] = str(i+1) + ' Skewness of aspect ratio'
        elif old_legend[i] == ' Rectangularity Mean ':
            new_legend[i] = str(i+1) + ' Mean of rectangularity'
        elif old_legend[i] == ' Rectangularity Variance ':
            new_legend[i] = str(i+1) + ' Variance of rectangularity'
        elif old_legend[i] == ' Rectangularity Skewness ':
            new_legend[i] = str(i+1) + ' Skewness of rectangularity'
        elif old_legend[i] == ' Solidity Mean ':
            new_legend[i] = str(i+1) + ' Mean of solidity'
        elif old_legend[i] == ' Solidity Variance ':  
            new_legend[i] = str(i+1) + ' Variance of solidity'
        elif old_legend[i] == ' Solidity Skewness ': 
            new_legend[i] = str(i+1) + ' Skewness of solidity'
        

    #print('new legend:', new_legend)
    #new_labels = ['Reference state', 'Unidirectional inclusion growth by 20\%' ]
    new_label = label
    if label == 'REF':
        new_label = 'Reference state'
    elif label == 'ISO-10':
        new_label = 'Isotropic inclusion growth by 10%'
    elif label == 'ISO-20':
        new_label = 'Isotropic inclusion growth by 20%'
    elif label == 'ANI-10':
        new_label = 'Unidirectional inclusion growth by 10%'
    elif label == 'ANI-20':
        new_label = 'Unidirectional inclusion growth by 20%'
    elif label == 'DEL-10':
        new_label = 'Inclusion deletion by 10%'
    elif label == 'DEL-20':
        new_label = 'Inclusion deletion by 20%'
    for i, (x_val, y_val) in enumerate(zip(x, corr_list_spearman), 1) :
        print('current label = ', label, ' correlation coefficient = ', y_val)
        if i == len(corr_list_spearman) - 1 :
            plt.plot(x_val, y_val, marker = marker, color=color, label=new_label, markersize = 8, linestyle = 'None', alpha = 0.7) # linestyle='--'
        else:
            plt.plot(x_val, y_val, marker = marker, color=color, markersize = 8, linestyle = 'None', alpha = 0.7)
    ...
    #plt.legend(new_legend, loc='best', bbox_to_anchor=(1, 0.5))
    plt.legend(fontsize = 15)
    textspace = 0.95
    for i in range(len(new_legend)):
        if new_legend[i] == str(i+1) + ' Frobenius norm of chord-length probability density \n function':
            textspace -= 0.06
        plt.text(21.5, textspace, new_legend[i] + '\n', fontsize=16)
        textspace -= 0.07
    # Write the decriptor index as textbox, not legend

    
    plt.xlabel('Microstructure descriptor index')
...
def compute_comparison(fen, trial1, trial2) :
    ...
    #print('tensors trial 1', tensors_trial1, ' \n', 'tensosr trial 2', tensors_trial2)
    to_compare = []
    ...
    for m in range(3) :
        for n in range(3) :
            a, b = indexm_to_indext(m,n)
            coefficient = 'C' + str(a) + str(b)
            # Here I am changing the script to do the error propagation analysis only for the significant stiffness tensor components,
            # C1111, C1122, C2222, C2121
            if coefficient == 'C1111' or coefficient == 'C2211' or coefficient == 'C2222' or coefficient == 'C2121':
...
def evolution_stiffness(fen, trial1) :
    ...
    #['ANI-20', 'ISO-20', 'DEL-20']
    colors = ['mediumturquoise', 'dodgerblue', 'darkseagreen', 'seagreen', 'darksalmon', 'indianred']
    colors = ['dodgerblue', 'seagreen','indianred']
    markers = ['.', 'o', 'v', '^', 's', 'p']
    markers = ['^','o','p']
    ...
    while again :
        ...
        # adding horizontal lines below correlation of 0.3 magnitude
        ...
        name = trial1.split('/')[-2] + '-EVOL-' + coeff + '.pdf'
        path = os.path.join('/Users/ag2556/Documents/CornellPostdoc/Cornell_Projects/MicroDescriptors/revised_correlation_anal/Graphs folder', name)
        #plt.ylim([-0.5, 1])# added
        plt.tight_layout()
...
def compare_errorpropag(fen, trial1, trial2) :
    ...
    while again :
        ...
        #dpi = 300.0  # increased dpi from 100 to 300
        ...
        name += '.pdf' # changed to be saved as pdf
