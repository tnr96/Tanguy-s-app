...
working_dir = '/Users/ag2556/Documents/CornellPostdoc/Cornell_Projects/MicroDescriptors/implementation/Tanguy-s-app-split-vrs'
moose_dir = '/Users/ag2556/projects/MicroDescriptors' # inside there I created a symbolic link to the solid mechanics moose executable, named as solid_mechanics
...
simulation_dir = working_dir + '/simulation_results'
...
destination_coreform = destination_moose  # run simulations on uniform meshes by moose instead, since adaptive refinement with coreform too time consuming
...
cutoff_input = 200 # I changed since now all images are 400 pixels
...
def mesh_images(images) :
    ...
    for image in images :
        ...
        convert_moose()
        #journal(image) # I commented those lines since I no longer use Cubit
        #to_copy(image) # I commented this line since I no longer use Cubit
...
def is_consistent(t1, t2, e) :
    ...
    # Comment out the consistency check, keep only the symmetry check
    '''        
    for i in range(shape[0]) :
        for j in range(shape[1]) :
    ...
    '''
    return_code = 2 # Return only the stiffness, computed from displacement BCs, which corresponds to index 2
...
def compute_cell(dir) :
    ...
    if len(files) < 3 : # decreased to 3 from 6 since only strain control
    ...
    for file in files :
        ...
        #print('related elements of file', name[-1], name[-2])  
        ...
        with open(file) as f :
            ...
            loading_case = name[1]
            #loading_case = name[-2]
            print('loading case ' , loading_case)
    ...
    # Do not solve for C_t, but instead set C_t = C_d
    '''
    S = solve_system_2D(eps_xx_t, eps_yy_t, [e * 2 for e in eps_xy_t], sig_xx_t, sig_yy_t, sig_xy_t)
    C_t = np.linalg.inv(S)
    '''
    C_t = C_d # just to avoid errors in other parts, use C_d only
    ...
    # I adjusted naming convention based on how my RVE data was named
    #txt_name = name[0] + '_' + name[1] + '_' + name[2] + '_effective_stiffness'
