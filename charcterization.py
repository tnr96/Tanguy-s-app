...
def compute_correlation(fen) :
    ...
    overwrite_bool = True
    if len(already_done_s2) > 0 or len(already_done_c) > 0 or len(already_done_lp):        
        overwrite_bool = overwrite(fen)
...
def main () :
    ...
    compute_correlation_btn = tk.Button(fen, text='Compute S2, C, and L', command= lambda : compute_correlation(fen))
    compute_correlation_btn.pack()
   
    plot_correlation_btn = tk.Button(fen, text='Plot S2, C, and L for a given image', command=plot_correlation_img)
